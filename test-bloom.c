...
{
    /* uint32_t values are max 10 characters and we reserve 1 bytes for the
     * separator or final nul-bytes.
     */
    ssize_t strbuflen = 11 * len;
    /* allocate 2 bytes extra for '[' and ']' characters */
    char *strbuf = malloc(strbuflen + 2);
    char *retval = strbuf;
    *strbuf = '[';
    strbuf++;
...
static char *size_t_array_check(size_t count, size_t *arr, ...)
{
    size_t ref[count];
    bool mismatch = false;
    va_list refv;
...
#define ck_assert_size_t_array_eq(len, arr, ...)                            \
    {                                                                       \
        ...
        (mismatched_other)                                                  \
            ? ck_abort_msg("%s != %s", size_t_array_to_string(arr, len),    \
                           mismatched_other)                                \
            : _mark_point(__FILE__, __LINE__);                              \
    }

/* Some helpers to call the bloom functions with only a single uint32_t as hash
 * value.
 */
static void bloom_set_single(byte_slice_t filter,
                             uint32_t value,
                             uint32_t num_bits)
{
...
static bool bloom_is_set_single(byte_slice_t filter,
                                uint32_t value,
                                uint32_t num_bits)
{
...
static void bloom_determine_offsets_single(size_t *bit_offsets,
                                           size_t num_bits,
                                           size_t filtersize,
                                           uint32_t value)
{
    bloom_determine_offsets(bit_offsets, num_bits, filtersize,
                            byte_slice_from_scalar(value));
}
...
START_TEST(test_filter_basics)
{
    ...
    /* Adding the other value should not influence the first one and the new one
     * should be present */
...
{
    uint8_t filter_data[1] = {
        0}; /* we use an insanely small bloom filter of just 8-bits */
    byte_slice_t filter = byte_slice_from_array(filter_data);
    ck_assert_int_eq(bloom_nr_bits_set(filter), 0);
...
START_TEST(test_filter_fill)
{
    ...
    uint32_t values[10] = {0,          10,        42,         1337,
                           65535,      65536,     1213141516, 0xdeadbeef,
                           0x99c0ffee, 0xffffffff};
    ...
    /* Due to the nature of the picked numbers the number of bits and
     * approximate counts are way off.
     */
...
START_TEST(test_filter_fill_random)
{
    ...
    /* The random numbers display a perfect bloom filter distribution and count
     * approximation.
     */
